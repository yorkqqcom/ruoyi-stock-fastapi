    ...
    try:
        ...
        async def run_async():
            async with ThreadSessionLocal() as db:
                try:
                    logger.info(f'开始执行回测任务，任务ID：{task_id}')
                    await BacktestService.run_backtest_task(db, task_id)
...
def execute_backtest_task(db_session_factory: Any, task_id: int) -> None:
    """
    在后台线程中执行回测任务
    ...
    thread = threading.Thread(
        target=run_backtest_task_sync,
        args=(db_session_factory, task_id),
        daemon=True,
        name=f'BacktestTask-{task_id}',
    )
    thread.start()
    logger.info(f'回测任务已在后台线程启动，任务ID：{task_id}')
